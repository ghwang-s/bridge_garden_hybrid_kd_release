...
class SFTConfig(TrainingArguments):
    ...
    gamma: float = field(
        default=0.9999,
        metadata={"help": "Gamma hyperparameter used by training or custom loss (float)."},
    )

    alpha: float = field(
        default=0.05,
        metadata={"help": "Alpha hyperparameter used in phi loss."},
    )

    distill_alpha: float = field(
        default=1.0,
        metadata={"help": "Alpha hyperparameter used by AB divergence distillation loss (float)."},
    )
    distill_beta: float = field(
        default=0.0,
        metadata={"help": "Beta hyperparameter used by AB divergence distillation loss (float)."},
    )
    distill_weight: float = field(
        default=0.5,
        metadata={"help": "Weight for combining distillation loss with original loss (float)."},
    )
    curriculum_warmup_ratio: float = field(
        default=0.2,
        metadata={
            "help": "Ratio of total training steps for curriculum learning warmup. During warmup period, distill_weight "
            "linearly increases from 0 to the target distill_weight. After warmup, it stays at the target value. "
            "Only used when phi_type='curriculum_weight'. Range: (0.0, 1.0]. For example, 0.2 means distill_weight "
            "grows from 0 to max in the first 20% of training, then stays at max for the remaining 80%."
        },
    )
    entropy_gamma: float = field(
        default=0.1,
        metadata={
            "help": "Gamma for power sum in entropy-based weighting. Only used when phi_type='entropy_sft_distill' or 'entropy_sft_sft'. Range: (0, 1)."
        },
    )
    entropy_reg_weight: float = field(
        default=0.1,
        metadata={
            "help": "Weight for student entropy regularization. Only used when phi_type='soft_kd_entropy_reg'."
        },
    )
    temperature_min: float = field(
        default=1.0,
        metadata={
            "help": "Minimum temperature used by temperature scheduling KD baselines. Must be > 0."
        },
    )
    temperature_max: float = field(
        default=4.0,
        metadata={
            "help": "Maximum temperature used by temperature scheduling KD baselines. Must be >= temperature_min."
        },
    )
    kd_reg_warmup_ratio: float = field(
        default=0.1,
        metadata={
            "help": "Ratio of total training steps to warm up the regularization term (reg_term) in KD loss. "
            "During warmup, reg_term weight linearly increases from 0 to 1. This prevents training instability "
            "caused by reg_term gradients when model logits are initially extreme (e.g., bf16 models). "
            "Set to 0 to disable warmup. Range: [0.0, 1.0]."
        },
    )
    phi_type: Optional[str] = field(
        default=None,
        metadata={
            "help": (
                "Phi function type used by training or custom loss. "
                "Options: None, 'random_label', 'chi2', 'chi2_curriculum', 'reverse_curriculum_weight', "
                "'soft_kd_entropy_reg', 'temperature_schedule_high_to_low', 'temperature_schedule_low_to_high', "
                "'forward_kl', 'reverse_kl', 'hellinger', 'total_variation', 'js'."
            )
        },
    )
    div: Optional[str] = field(
        ...
        metadata={
            "help": (
                "Options: 'ab', 'tv', 'sfkl', 'srkl', 'akl', 'js'. "
            )
        },
    )
    ...
    def __post_init__(self):
        ...
        if isinstance(self.phi_type, str) and self.phi_type.strip().lower() in {"none", "null"}:
            self.phi_type = None
