...
class GKDConfig(SFTConfig):
    ...
    lambda_fkl: float = field(
        default=1.0,
        metadata={"help": "Weight for the KD loss term. Used by hybrid on-policy GKD variants."},
    )
    lambda_sft: float = field(
        default=0.0,
        metadata={"help": "Weight for the hard-label loss term. Used by hybrid on-policy GKD variants."},
    )
    ...
    alpha: float = field(
        default=0.05,
        metadata={"help": "Alpha hyperparameter used by training or custom loss (float)."},
    )
    phi_type: Optional[str] = field(
        ...
        metadata={
            "help": (
                ...
                "Options: None, 'chi2', 'forward_kl', 'reverse_kl', 'hellinger', 'total_variation', 'js'."
            )
        },
    )
